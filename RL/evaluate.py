...
class ScenarioSpec:
    ...
    def to_obs(self) -> np.ndarray:
        ...
        for i in range(13):
            obs[13 + i] = 1 if ri == i else 0
        obs[26] = np.sin(2 * np.pi * ri / 13) / 2 + 0.5
        obs[27] = np.cos(2 * np.pi * ri / 13) / 2 + 0.5
        obs[28] = self.pile_size / 52.0
        obs[29] = self.claim_qty / 4.0
        obs[30] = 0.0 if self.phase == "DECLARE" else 1.0
...
SCENARIOS: Dict[str, ScenarioSpec] = {
    ...
    "large_pile_high_claim": ScenarioSpec(
        hand=["A","5","7","9"], current_rank="A",
        phase="CHALLENGE", pile_size=30, claim_qty=3,
    ),
}
